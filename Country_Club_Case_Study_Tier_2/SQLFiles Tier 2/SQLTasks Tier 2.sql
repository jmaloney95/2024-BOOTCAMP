...
SELECT priceCategory, name, monthlymaintenance
FROM Facilities
...
SELECT firstname, surname, MAX(joindate) AS last_signup
FROM Members;
...
SELECT DISTINCT CONCAT(f.name, ': ', m.firstname, '', m.surname) AS CourtAndMember
FROM Members AS m
...
SELECT f.facid, CONCAT(m.firstname, ' ', m.surname) AS MemberName, f.membercost AS Cost
FROM Bookings AS b
...
WHERE b.starttime >= '2012-09-14 00:00:00' 
    ...
    AND f.membercost > 30
UNION ALL
SELECT f.facid, CONCAT(m.firstname, ' ', m.surname) AS MemberName, f.guestcost AS Cost
FROM Bookings AS b
INNER JOIN Facilities AS f
ON f.facid = b.facid
WHERE b.starttime >= '2012-09-14 00:00:00' 
...
SELECT f.facid, CONCAT(m.firstname, ' ', m.surname) AS MemberName, 
FROM Bookings AS b    
    (SELECT 
        CASE 
...
SELECT m1.surname AS MemberSurname, m1.firstname AS MemberFirstname,
...
SELECT f.name AS FacilityName, COUNT(b.bookid) AS UsageCount
FROM Bookings AS b
INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE b.memid != 0
GROUP BY f.name
